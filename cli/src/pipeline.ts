import {
  loadConfig, ProgressEmitter, TokenBudget,
  createFileSkillProvider, createBundledSkillProvider, getBundledSkillsDir,
  createDebugLogger, RunContextBuilder, saveRunContext, loadRunContext,
  Checkpoint,
  writePreAgentSnapshotSidecar,
  PHASE_ORDER,
  assessBaasAppropriateness,
  type AgentConfigType, type AppContext, type BackendContext, type BaasProvider, type BaasRecommendation, type DtcConfig, type PhaseId, type Platform, type RunMode, type SkillContent, type DebugLogger, type PlatformSpec,
} from '@appifex/core'
import { createRunner } from '@appifex/runner'
import { createDesignAdapter, PencilMcpClient, writeImageAssets, extractStitchZip, type StitchArtifacts } from '@appifex/design'
import type { DesignToolResult } from '@appifex/core'
import { extractSpec, extractSpecFromMcp, translateSpec, generateSpecFromPrompt, extractSpecFromStitch, extractSpecFromFigmaMake } from '@appifex/spec'
...
import { ClaudeAdapter, createDefaultGenerateFn, createClaudeCliGenerateFn, createLayeredGenerateFn, type GenerateFn, type LayeredCodegenResult } from '@appifex/codegen'
import { buildSwift, buildKotlin, archiveSwift, bundleKotlin, deriveAppName, swiftPrecheck, swiftAutofix, patchProjectDependencies, patchBuildGradle } from '@appifex/build'
...
function isLayeredCodegenResult(result: { files: unknown[] }): result is LayeredCodegenResult {
  return 'presentationFiles' in result
    && 'domainFiles' in result
    && 'integrationFiles' in result
}

/** Map CLI Platform to BaaS TargetPlatform array. */
function platformToTarget(platform: Platform | undefined): import('@appifex/baas').TargetPlatform[] {
...
/** @internal */
export function decidePenFileStrategy(prompt: string, appContext: import('@appifex/core').AppContext | null): PenFileDecision {
  ...
  if (existingScreenNames.length > 0 && existingScreenNames.some(name => promptLower.includes(name)) && !mentionsNewScreen) {
    return 'extend'
  }
...
export function generatePreBuildSummary(
  ...
  const COMMON_WORDS = new Set([
    'the', 'and', 'for', 'with', 'add', 'new', 'create', 'build',
    'that', 'from', 'this', 'screen', 'page', 'view', 'tab',
  ])
...
export async function snapshotSourceFiles(
  ...
  for (const filePath of allFiles) {
    ...
    if (filePath.includes('__tests__/') || filePath.includes('build/') || filePath.includes('.dtc-debug/') || filePath.includes('node_modules/')) {
      continue
    }
    try {
...
export async function runTestRegenPhase(deps: RunTestRegenDeps): Promise<void> {
  const {
    runner, outputDir, platform, runMode, preAgentSnapshot,
    platformSpec, ctxBuilder, emit, flushContext,
    flowDir, testDir, bundleId, designScreenshots,
  } = deps

  // Pitfall 6 double-gate: fresh-app runs must not reach the diff.
  if (runMode !== 'add-feature' || !preAgentSnapshot) return

  const modifiedScreens = await diffScreenInventory(
    outputDir,
    platform,
    runner,
    preAgentSnapshot,
  ...
  const screenFilter = new Set<string>([
    ...modifiedScreens.added,
    ...modifiedScreens.modified,
  ])
 ...
 */
export async function runDesignDeltaPhase(deps: RunDesignDeltaDeps): Promise<DesignDeltaReport | null> {
  const {
    runMode, existingDesignTokens, newDesignTokens,
    ctxBuilder, emit, interactive, readline, acceptDrift,
  } = deps
  ...
  if (interactive !== false) {
    ...
    if (hasDrift && acceptDrift !== true) {
      ...
      emit('design_delta', 'failed', `Design drift detected (${c} changed, ${r} removed) — fail-closed in non-interactive mode.`)
      throw new Error(
...
export async function runPipeline(
  ...
  const outputDir = opts.outputDir.startsWith('/') ? opts.outputDir : join(process.cwd(), opts.outputDir)
  ...
  async function buildCreateMessageFn(cfg: DtcConfig) {
    ...
    if (cfg.llm.provider === 'claude-cli') {
      ...
      return async (params: { model: string; max_tokens: number; messages: Array<{ role: string; content: any }> }) => {
        ...
        return new Promise<{ content: Array<{ type: string; text: string }>; usage: { input_tokens: number; output_tokens: number } }>((resolve, reject) => {
          ...
          child.on('close', (code) => {
            if (code !== 0) reject(new Error(`claude --print exited with code ${code}${stderr ? `\nstderr: ${stderr}` : ''}${stdout ? `\nstdout preview: ${stdout.slice(0, 200)}` : ''}`))
            else if (!stdout.trim()) reject(new Error(`claude --print returned empty response${stderr ? `\nstderr: ${stderr}` : ''}`))
            else resolve({ content: [{ type: 'text', text: stdout }], usage: { input_tokens: 0, output_tokens: 0 } })
          })
        })
      }
    }
    ...
    if (cfg.llm.provider === 'copilot' && cfg.llm.githubToken) {
      ...
      return async (params: { model: string; max_tokens: number; messages: Array<{ role: string; content: any }> }) => {
        ...
        const client = new CopilotClient({ githubToken: cfg.llm.githubToken, useLoggedInUser: false }) as any
        await client.start()
        try {
          const session = await client.createSession({ model: cfg.llm.model ?? 'claude-sonnet-4-6', onPermissionRequest: () => ({ allow: true }) })
    ...
    if (cfg.llm.provider === 'google') {
      return async (params: { model: string; max_tokens: number; messages: Array<{ role: string; content: any }> }) => {
        ...
        return { content: [{ type: 'text', text }], usage: { input_tokens: inputTokens, output_tokens: outputTokens } }
      }
    }
    ...
    if (cfg.llm.provider === 'openai') {
      ...
      return async (params: { model: string; max_tokens: number; messages: Array<{ role: string; content: any }> }) => {
        ...
        return { content: [{ type: 'text', text }], usage: { input_tokens: data.usage?.prompt_tokens ?? 0, output_tokens: data.usage?.completion_tokens ?? 0 } }
      }
    }
    ...
    return async (params: { model: string; max_tokens: number; messages: Array<{ role: string; content: any }> }) => {
      ...
      return {
        content: finalMessage.content.map((c: any) => ({ type: c.type, text: c.type === 'text' ? c.text : '' })),
  ...
  let effectivePrompt = (opts.prompt === RESUME_PLACEHOLDER && previousContext?.prompt)
    ? previousContext.prompt
    : opts.prompt
  const ctxBuilder = new RunContextBuilder({ prompt: effectivePrompt, platform: opts.platform, mode: runMode })
  ...
  if (detectedProvider) {
    progress.emit({ phase: 'baas_recommend', status: 'running', message: `Auto-detected BaaS provider: ${detectedProvider}`, timestamp: Date.now() })
  }
  let resolvedBaasProvider: BaasProvider | null = opts.baasProvider ?? detectedProvider ?? config.baas?.provider ?? null

  const emit = (phase: PhaseId, status: 'started' | 'running' | 'completed' | 'failed' | 'skipped', message: string, tokens?: number) => {
  ...
  const flushContext = async (status: 'failed' | 'completed' | 'budget_exceeded' = 'failed') => {
    try { await saveRunContext(outputDir, ctxBuilder.build(status)) } catch { /* must not block pipeline */ }
  }
  ...
  const checkpointRunId = (isContinuation && previousContext?.runId)
  ...
  const exitCleanup = () => { try { checkpoint.close() } catch { /* ignore */ } }
  ...
  const sigintHandler = () => {
    ...
    try {
      ...
      writeFileSyncFs(join(ctxDir, 'run-context.json'),
        JSON.stringify(ctxBuilder.build('failed'), null, 2))
    } catch { /* best effort */ }
    try { checkpoint.close() } catch { /* best effort */ }
  ...
  try {
    resumeState = await runResumeBootstrap({
      ...
      checkpoint,
      checkpointRunId,
      logBanner: (msg) => { console.error(msg) },
    })
  } catch (err) {
  ...
  if (runMode === 'add-feature' && !canSkipPhase('analysis')) {
    ...
    try {
      ...
      ctxBuilder.recordPhase('analysis', 'completed',
        ...
        { appContext })
      emit('analysis', 'completed', `Found ${inventory.filter(e => e.type === 'screen').length} screens, ${navResult.nodes.length} nav nodes`)
    } catch (err) {
    ...
    emit('analysis', 'skipped', `Resumed from checkpoint (lastCompleted=${resumeState.lastCompletedPhase})`)
  ...
  if (runMode === 'add-feature') {
    ...
    if (isFeaturePromptVague(effectivePrompt)) {
      ...
      if (isInteractive(opts)) {
        ...
        if (!hasKeywords(effectivePrompt, ['screen', 'page', 'view', 'tab', 'button', 'action', 'list', 'section', 'modal', 'sheet', 'form', 'input'])) {
          missingDims.push('Missing: what type of element to add (screen, button, tab, etc.)')
        }
        if (!hasKeywords(effectivePrompt, ['to', 'on', 'in', 'inside', 'from', 'navigation', 'tab bar', 'home', 'settings', 'profile'])) {
          missingDims.push('Missing: where the element goes (which screen or nav target)')
        }
        if (!hasKeywords(effectivePrompt, ['show', 'display', 'let', 'allow', 'enable', 'create', 'add', 'delete', 'edit', 'update', 'save'])) {
        ...
        if (answer.toLowerCase() === 'n') {
          ...
          console.log(chalk.yellow('-> Describe what to add, where it goes, and what it does, then run again.'))
          throw new Error('Pipeline cancelled. Provide a more specific prompt and try again.')
        }

        // Bake assumptions into prompt
        effectivePrompt = [effectivePrompt, ...assumptionResult.assumptions.map(a => a.description)].join('. ')
      } else {
  ...
  if (runMode === 'add-feature' && appContext) {
    if (resumeState !== null && previousContext?.modificationPlan !== undefined) {
      ...
      emit('analysis', 'running', `Rehydrated modification plan from previous run (${modificationPlan.items.length} items)`)
    } else {
      ...
      try {
        ...
        emit('analysis', 'running', `Modification planning failed: ${String(err)} — proceeding without modifications`)
      ...
      if (!alreadyIncluded) {
        try {
          ...
          modificationPlan.items.push({
            ...
            screenName: 'AppEntry',
            changeDescription: 'Add AuthManager environment object, wrap content in auth gate (show LoginView when unauthenticated), add deep link handling via onOpenURL',
            changeType: 'navigation',
            fileContent: existingContent,
          })
          ctxBuilder.setModificationPlan(modificationPlan)
          emit('analysis', 'running', `Injected AppEntry.swift into modification plan for BaaS auth guard merge`)
        } catch {
  ...
  if (runMode === 'add-feature') {
    ...
    if (penExists) {
      try {
        ...
        existingDesignTokens = existingSpec.designTokens ?? { colors: {}, typography: {}, spacing: {}, borderRadius: {} }
        ...
        emit('design', 'running', `${colorCount + spacingCount} tokens loaded — colors, typography, spacing`)
      } catch (err) {
    ...
    if (previousContext?.baselineDesignTokens) {
      ...
      emit('design', 'running', `baseline tokens loaded from previous run (${Object.keys(baselineDesignTokens.colors).length} colors)`)
    } else {
  ...
  let penStrategy: 'new' | 'extend' = appContext
  ...
  if (opts.designFile) {
    ...
    const userPath = opts.designFile.startsWith('/') ? opts.designFile : join(process.cwd(), opts.designFile)
    ...
    if (userPath.endsWith('.zip')) {
      ...
      emit('design', 'completed', `Using Stitch export: ${opts.designFile} (${stitchArtifacts.htmlPaths.length} screen(s))`)
    ...
    const designPrompt = adapter.tool === 'pencil' ? buildPencilPrompt(effectivePrompt, opts.platform) : effectivePrompt
    const startMsg = adapter.tool === 'pencil'
    ...
    if (runMode === 'add-feature' && appContext) {
      if (penStrategy === 'extend') {
        ...
        lastDesignResult = await adapter.iterate({ prompt: designPrompt, outputDir, inputPath: designPath, outputPath: designPath, previewPath })
      } else {
    ...
    while (!approved) {
      ...
      if (action === 'approve') {
        ...
        const iteratePrompt = await clack.text({
          ...
          placeholder: 'e.g. Make cards 2-column grid, add priority color badges, bigger add button',
        ...
        emit('design', 'completed', `Design updated (iteration ${designIterations}) — review and approve`)
      }
    }
    ...
    try {
      ...
      if (layout.problems.length > 0) {
        ...
        if (isInteractive(opts)) {
          ...
          emit('spec', 'running', 'Non-interactive mode: skipping layout fix (continuing with warnings)')
        }
      }
    } catch {
  ...
  if (canSkipPhase('spec') && await runner.exists(specPath)) {
    ...
    if (saved?.platformSpec) {
      ...
      progress.emit({ phase: 'spec', status: 'skipped', message: 'Using spec from previous run', timestamp: Date.now() })
  ...
  if (stitchArtifacts && adapter.tool === 'figma-make') {
    ...
    const codeContent = stitchArtifacts.htmlPaths.length > 0
    ...
    try {
      ...
      if (mcpClient) {
        try {
          ...
          emit('spec', 'running', `MCP spec extraction failed: ${mcpMsg.slice(0, 120)} — falling back to LLM`)
          await debug.log('spec-mcp-error.txt', `MCP spec extraction failed:\n${mcpMsg}\n\nStack: ${mcpErr instanceof Error ? mcpErr.stack ?? '' : ''}`)
        }
      } else {
      ...
      if (!spec) {
        ...
        emit('spec', 'running', `Generating spec via LLM (${config.llm.provider}/${config.llm.model ?? 'default'})`)
        const createMessage = await getCreateMessage()
        // claude-cli provider (claude --print) is text-only — skip design image
        const canSendImages = config.llm.provider !== 'claude-cli'
        const specDesignImage = canSendImages && await runner.exists(previewPath) ? previewPath : undefined
        try {
          ...
          await debug.log('spec-llm-error.txt', `LLM spec generation failed:\n${llmMsg}\n\nStack: ${llmErr instanceof Error ? llmErr.stack ?? '' : ''}`)
          throw llmErr
        }
      }
    }
  ...
  emit('spec', 'completed', `${platformSpec.screens.length} screens, ${opts.platform}`, specTokens)
  ...
  if (runMode === 'add-feature') {
    ...
    const nextTokens = platformSpec.designTokens ?? { colors: {}, typography: {}, spacing: {}, borderRadius: {} }
    ...
    if (designDelta !== null) {
      ...
      emit('design_delta', 'skipped', `Resumed from checkpoint (lastCompleted=${resumeState?.lastCompletedPhase ?? 'unknown'}, designDelta rehydrated=${designDelta !== null})`)
    }
    ...
    if (isInteractive(opts)) {
      ...
      emit('design', 'running', `Pre-build plan: ${summary.newScreens.length} new screen(s), ${summary.modifiedFiles.length} file(s) to modify`)
      ...
      emit('design', 'running', `Design: ${summary.designStrategy === 'new' ? 'create new .pen file' : 'extend existing .pen file'}`)
    ...
    if (resolvedBaasProvider !== null && !canSkipPhase('baas_recommend')) {
      ...
      if (existingBaas?.provider && existingBaas.provider !== resolvedBaasProvider) {
        const hasDownstreamBaas = existingBaas.schema !== undefined || existingBaas.authConfig !== undefined
        if (hasDownstreamBaas) {
          ...
          throw new Error(`BaaS provider switch blocked: ${existingBaas.provider} → ${resolvedBaasProvider}`)
        }
      }

      // Resolve PlatformSpec — may be null on --resume if spec phase was skipped
      const specForHeuristic = platformSpec
        ?? (previousContext?.phases?.spec?.detail as { platformSpec?: PlatformSpec } | undefined)?.platformSpec
        ?? null
    ...
    if (resolvedBaasProvider !== null && !canSkipPhase('baas_schema')) {
      ...
      const { inferBaasSchema, renderBaasTemplates, lintSecurityRules, generateSdkInit, generateConfigStubs, generateDataServices } = await import('@appifex/baas')
      ...
      const baasFiles = renderBaasTemplates(schema, resolvedBaasProvider, platformToTarget(opts.platform as Platform | undefined))

      // D-10: Security lint gate
      const securityFiles = baasFiles.filter((f: { path: string; content: string }) => f.path.endsWith('.rules') || f.path.endsWith('.sql'))
      ...
      const dataServiceFiles = generateDataServices(schema, resolvedBaasProvider, platformToTarget(opts.platform as Platform | undefined))
      ...
      ctxBuilder.setBaasContext({
        ...existingBaas,
        schema,
        securityRules: securityFiles.map((f: { path: string; content: string }) => f.content).join('\n\n'),
      })
      await flushContext()
    ...
    if (resolvedBaasProvider !== null && !canSkipPhase('baas_auth')) {
      ...
      const authFiles = generateAuthTemplates(resolvedBaasProvider, platformToTarget(opts.platform as Platform | undefined), baasSchema)

      // D-08: Deep link configuration stubs
      const deepLinkFiles = resolvedBaasProvider !== 'mock'
        ? generateAuthDeepLinkConfig(resolvedBaasProvider)
        : []
    ...
    if (resolvedBaasProvider === 'mock' && !canSkipPhase('mock_service')) {
      ...
      const { renderBaasTemplates: renderMock, generateAuthTemplates: genAuthMock, generateDataServices: genDataMock } = await import('@appifex/baas')
  ...
  if (canSkipPhase('test_gen') && await runner.exists(flowDir) && await runner.exists(testDir)) {
    ...
    if (saved) {
      uiTests = saved.uiTestFileNames.map(fn => ({ name: fn, screenId: '', fileName: fn, content: '' }))
      unitTests = saved.unitTestFileNames.map(fn => ({ fileName: fn, content: '', platform: opts.platform, testCount: 0 }))
      // Emit to progress system but record as 'completed' so next resume still skips
      progress.emit({ phase: 'test_gen', status: 'skipped', message: 'Using tests from previous run', timestamp: Date.now() })
  ...
  if (stitchArtifacts && stitchArtifacts.screenshotPaths.length > 0) {
    ...
    for (let i = 0; i < platformSpec.screens.length && i < stitchArtifacts.screenshotPaths.length; i++) {
  ...
  emit('test_gen', 'completed', `${uiTests.length} UI flows, ${totalUnitTests} unit tests (locked)`)
  ...
  if (agentType !== 'api') {
    const { createAgent, detectAgent, createProgressParser, buildAgentPrompt } = await import('@appifex/agent')
    ...
    if (agent) {
      ...
      if (runMode === 'add-feature') {
        if (resumeState !== null) {
          ...
          if (!canSkipPhase('codegen')) {
            ...
            emit('codegen', 'running', `Reverted ${revertedCount} file(s) to preAgent baseline for resumed codegen`)
          }
      ...
      while (result.stopReason === 'budget_exceeded' && isInteractive(opts)) {
        ...
        emit('codegen', 'running', `Budget exceeded (${costStr} spent). Agent has session ${result.sessionId?.slice(0, 8) ?? '?'}...`)
        ...
        const action = await clack.select({
          ...
          options: [
            { value: 'continue', label: `Add $${agentBudget} more and continue`, hint: 'resumes the same session' },
            { value: 'continue-large', label: `Add $${agentBudget * 3} more and continue`, hint: 'larger budget for complex apps' },
        ...
        emit('codegen', 'running', `Resuming agent session with $${additionalBudget} more budget...`)
      ...
      if (result.stopReason === 'budget_exceeded' && !isInteractive(opts)) {
        emit('codegen', 'running', 'Non-interactive mode: budget exceeded, keeping generated output')
      }
      ...
      if (result.costUsd) writeSync(join(debugDir, 'agent-cost.txt'), `$${result.costUsd.toFixed(4)}`)
      ...
      if (preAgentSnapshot && runMode === 'add-feature') {
        const revertedFiles = await revertUnexpectedChanges(runner, preAgentSnapshot, modificationPlan)
        if (revertedFiles.length > 0) {
          emit('codegen', 'running', `Reverted unexpected changes to ${revertedFiles.length} file(s): ${revertedFiles.join(', ')}`)
        }
      }
      ...
      if (result.success) {
        ...
        emit('security', 'started', `Scanning with ${opts.platform === 'swiftui' ? 'p/swift' : opts.platform === 'kotlin-compose' ? 'p/kotlin' : 'p/react'} rules`)
        ...
        if (secResult.error) {
          ...
          const secBuildFn = async () => opts.platform === 'kotlin-compose'
          ...
          const createMessage = await getCreateMessage()
          const secFixFn = config.llm.provider === 'claude-cli'
            ? createClaudeCliFixFn({ runner, projectDir: outputDir, model: config.llm.model })
            : createDefaultFixFn({ apiKey: config.llm.apiKey ?? '', runner, projectDir: outputDir, model: config.llm.model, createMessage, skillPrompt: skills.fixPrompt, verbose: opts.verbose })
          ...
          emit('fix', fr.status === 'all_green' ? 'completed' : 'failed',
            `${fr.status} — ${fr.attempts.length} attempts`, fr.totalTokensUsed)
          emit('security', fr.status === 'all_green' ? 'completed' : 'failed',
            fr.status === 'all_green' ? 'Fixed — clean' : 'Finding(s) remaining after fix loop')
        }
      } else if (result.stopReason === 'budget_exceeded') {
        const sessionHint = result.sessionId ? ` Resume with: dtc run --resume ${result.sessionId}` : ''
        emit('codegen', 'failed', `Budget exceeded (${result.costUsd ? '$' + result.costUsd.toFixed(2) : ''}).${sessionHint}`)
      } else {
      ...
      try {
        for (const pattern of ['Sources/**/*.swift', 'src/**/*.{ts,tsx}', '__tests__/**/*.swift', '__tests__/**/*.{ts,tsx}']) {
          const files = await runner.glob(`${outputDir}/${pattern}`)
          for (const f of files) {
            generatedFiles.push(f.replace(outputDir + '/', ''))
          }
        }
      } catch { /* glob failure shouldn't prevent report generation */ }
      ...
      const finalValidation = result.success
        ? { ui: { total: uiTests.length, passed: uiTests.length, failed: 0, results: [] }, unit: { total: agentUnitTestCount, passed: agentUnitTestCount, failed: 0, failures: [] }, allPassed: true } as ValidationResult
        : { ui: { total: 0, passed: 0, failed: 0, results: [] }, unit: { total: 0, passed: 0, failed: 0, failures: [] }, allPassed: false } as ValidationResult
      ...
      const runStatus = result.success ? 'completed' as const
        : result.stopReason === 'budget_exceeded' ? 'budget_exceeded' as const
        : 'failed' as const
      try { await saveRunContext(outputDir, ctxBuilder.build(runStatus)) } catch { /* context save must not block return */ }
  ...
  if (config.llm.provider === 'claude-cli') {
    ...
    const createMessage = await getCreateMessage()
    const layeredFn = createLayeredGenerateFn({ apiKey: config.llm.apiKey ?? '', model: config.llm.model, createMessage })
  ...
  emit('codegen', codegenResult.success ? 'completed' : 'failed',
    codegenResult.success ? `${codegenResult.files.length} files` : codegenResult.error ?? 'Failed',
  ...
  if (resolvedBaasProvider !== null && resolvedBaasProvider !== 'mock') {
    ...
    if (opts.platform === 'swiftui') {
      ...
      emit('build', 'running', `${resolvedBaasProvider === 'firebase' ? 'Firebase' : 'Supabase'} dependencies added to project.yml`)
    } else if (opts.platform === 'kotlin-compose') {
      await patchBuildGradle(runner, outputDir, resolvedBaasProvider)
      emit('build', 'running', `${resolvedBaasProvider === 'firebase' ? 'Firebase' : 'Supabase'} dependencies added to build.gradle.kts`)
    }
  }
  ...
  if (opts.platform === 'swiftui') {
    ...
    if (!precheck.passed) {
      ...
      if (fixedCount > 0) {
        emit('build', 'running', `Auto-fixed ${fixedCount} file${fixedCount !== 1 ? 's' : ''} (deprecated APIs)`)
      }
    }
  }
  ...
  const reportDir = join(outputDir, '.dtc-report')
  const buildFn = async () => opts.platform === 'kotlin-compose'
    ? buildKotlin(runner, { projectDir: outputDir })
    : buildSwift(runner, { projectDir: outputDir, scheme: appName })
  const validateFn = async () => validateAll(runner, {
    platform: opts.platform, projectDir: outputDir, flowDir, testDir, reportDir,
    ...(resolvedBaasProvider === 'mock' && {
  ...
  emit('build', buildResult.success ? 'completed' : 'failed',
    buildResult.success ? `${opts.platform} ✓` : buildResult.error?.split('\n').pop() ?? 'Build failed')
  await flushContext()
  ...
  if (!buildResult.success) {
    ...
    const buildFailValidation: ValidationResult = {
      ...
      unit: {
        ...
        failures: buildErrors.length > 0
          ...
          : [{ testName: 'build', suiteName: 'xcodebuild', error: buildResult.error ?? 'Build failed' }],
      },
      allPassed: false,
    }
    ...
    if (!opts.fixFn) {
      if (config.llm.provider === 'claude-cli') {
        opts.fixFn = createClaudeCliFixFn({ runner, projectDir: outputDir, model: config.llm.model })
      } else {
        opts.fixFn = createDefaultFixFn({ apiKey: config.llm.apiKey ?? '', runner, projectDir: outputDir, model: config.llm.model, createMessage, skillPrompt: skills.fixPrompt, verbose: opts.verbose })
      }
    }
  ...
  if (buildResult.success) {
    ...
    if (testsResolved) {
      const { runSemgrep } = await import('@appifex/validate')
      emit('security', 'started', `Scanning with ${opts.platform === 'swiftui' ? 'p/swift' : opts.platform === 'kotlin-compose' ? 'p/kotlin' : 'p/react'} rules`)
      const secResult = await runSemgrep(runner, { projectDir: outputDir, platform: opts.platform })
      if (secResult.error) {
        emit('security', 'failed', secResult.error)
        await flushContext()
      } else if (secResult.findings.length === 0) {
        ...
        emit('fix', fr.status === 'all_green' ? 'completed' : 'failed',
          `Security fix ${fr.status} — ${fr.attempts.length} attempts`, fr.totalTokensUsed)
        await flushContext()
        emit('security', fr.status === 'all_green' ? 'completed' : 'failed',
          fr.status === 'all_green' ? 'Fixed — clean' : 'Finding(s) remaining after fix loop')
        await flushContext()
        ...
        const finalSec = await runSemgrep(runner, { projectDir: outputDir, platform: opts.platform })
        validation = { ...validation, security: finalSec, allPassed: validation.allPassed && finalSec.failed === 0 }
      }
    } else {
    ...
    validation = { ui: { total: 0, passed: 0, failed: 0, results: [] }, unit: { total: 0, passed: 0, failed: 0, failures: [] }, allPassed: false }
  ...
  if (deliverConfig && report.summary.allGreen) {
    ...
    try {
      ...
      if (!deliverConfig?.remoteUrl) {
        try {
          ...
          const nameResp = await createMessage({
            ...
            max_tokens: 30,
            messages: [{ role: 'user', content: `Generate a short GitHub repository name (lowercase, hyphens, no spaces, max 3 words) for this app:\n\n"${opts.prompt}"\n\nRespond with ONLY the repo name, nothing else.` }],
          })
          const raw = nameResp.content[0]?.text?.trim().toLowerCase().replace(/[^a-z0-9-]/g, '-').replace(/-+/g, '-').replace(/^-|-$/g, '') ?? ''
          if (raw.length > 0 && raw.length <= 50) repoName = raw
        } catch {
          // Fall back to simple slugify if LLM fails
        }
        repoName ??= opts.prompt.toLowerCase().split(/[\s_]+/).slice(0, 3).join('-').replace(/[^a-z0-9-]/g, '').slice(0, 50) || 'dtc-app'
      }
      ...
      deliverResult = await deliver(runner, {
        ...
        git: deliverConfig ? { userName: deliverConfig.userName, userEmail: deliverConfig.userEmail } : undefined,
      ...
      const repoNote = deliverResult.repoCreated ? ` (repo created: ${deliverResult.remoteUrl})` : ''
  ...
  const hasFullAscCreds = config.apple?.ascKeyId && config.apple?.ascIssuerId && config.apple?.ascKeyPath && config.apple?.ascAppId
  const hasAndroidCreds = config.android?.serviceAccountKeyPath && config.android?.packageName && config.android?.keystorePath
  ...
  if (opts.platform === 'swiftui' && hasFullAscCreds && report.summary.allGreen) {
    ...
    try {
      ...
      if (!archiveResult.success) {
        ...
        const submitResult = await asc.submitTestFlight({ appId: config.apple!.ascAppId!, ipaPath: archiveResult.ipaPath!, group: config.apple!.ascTestFlightGroup })
    ...
    emit('provision', 'skipped', 'Google Play Console credentials not configured — run `dtc setup`')
    await flushContext()
  } else if ((opts.platform === 'swiftui' || opts.platform === 'kotlin-compose') && !report.summary.allGreen) {
  ...
  } catch { /* glob failure shouldn't prevent saving context */ }
  ctxBuilder.setFilesGenerated(apiFilesGenerated)
  const apiRunStatus = validation.allPassed ? 'completed' as const
    : fixResult?.status === 'budget_exceeded' ? 'budget_exceeded' as const
    : 'failed' as const
  try { await saveRunContext(outputDir, ctxBuilder.build(apiRunStatus)) } catch { /* context save must not block return */ }
...
function buildPencilPrompt(prompt: string, platform: import('@appifex/core').Platform): string {
  const platformGuidelines = platform === 'swiftui'
...
- Support for both compact and expanded screen sizes`
  ...
  const platformLabel = platform === 'swiftui' ? 'iOS native SwiftUI'
    : platform === 'kotlin-compose' ? 'Android native Kotlin Compose with Material Design 3'
...
function findImageComponents(spec: import('@appifex/core').PlatformSpec): Array<{ id: string; name: string }> {
...
function flattenPlatformComponents(comps: import('@appifex/core').PlatformComponentSpec[]): import('@appifex/core').PlatformComponentSpec[] {
