...
export async function handleLoadContext(args: { outputDir: string }): Promise<{ text: string; isError: boolean }> {
  const ctx = await loadRunContext(args.outputDir)
  if (!ctx) {
    return { text: JSON.stringify({ found: false, message: 'No run context found' }), isError: false }
  }
  return { text: JSON.stringify({ found: true, context: ctx }, null, 2), isError: false }
}

export async function handleSaveContext(args: { outputDir: string; contextJson: string }): Promise<{ text: string; isError: boolean }> {
  try {
    ...
    return { text: `Save context failed: ${err instanceof Error ? err.message : String(err)}`, isError: true }
  }
}
