...
function mockRunner(overrides: Partial<Runner> = {}): Runner {
  return {
    exec: vi.fn().mockResolvedValue({ exitCode: 0, stdout: '', stderr: '', duration: 1000, command: 'mock' }),
    readFile: vi.fn().mockResolvedValue(''),
    writeFile: vi.fn().mockResolvedValue(undefined),
    exists: vi.fn().mockResolvedValue(true),
    glob: vi.fn().mockResolvedValue([]),
    capabilities: { hasMaestro: false, hasXcode: false, hasNode: true, hasSemgrep: false, platform: 'linux' },
    ...overrides,
  }
}
...
describe('bundleKotlin', () => {
  it('runs gradlew bundleRelease and returns success with aabPath', async () => {
    const exec = vi.fn().mockResolvedValue({ exitCode: 0, stdout: 'BUILD SUCCESSFUL', stderr: '', duration: 5000, command: './gradlew bundleRelease --no-daemon' })
    const glob = vi.fn().mockResolvedValue(['/tmp/test-app/app/build/outputs/bundle/release/app-release.aab'])
...
android {
    ...
    const exec = vi.fn().mockResolvedValue({ exitCode: 0, stdout: '', stderr: '', duration: 1000, command: 'mock' })
    const glob = vi.fn().mockResolvedValue(['/tmp/test-app/app/build/outputs/bundle/release/app-release.aab'])
    const runner = mockRunner({ writeFile, readFile, exec, glob, exists: vi.fn().mockResolvedValue(true) })
    ...
    const signingWrite = writeFile.mock.calls.find(
      (c: unknown[]) => typeof c[0] === 'string' && c[0].includes('build.gradle.kts') && typeof c[1] === 'string' && c[1].includes('signingConfigs'),
    )
    ...
    const propsWrite = writeFile.mock.calls.find(
      (c: unknown[]) => typeof c[0] === 'string' && c[0].includes('gradle.properties') && typeof c[1] === 'string' && c[1].includes('RELEASE_STORE_FILE'),
    )
...
android {
  ...
  it('returns failure on non-zero exit code', async () => {
    const exec = vi.fn()
      .mockResolvedValueOnce({ exitCode: 0, stdout: '', stderr: '', duration: 100, command: 'chmod' }) // chmod
      .mockResolvedValueOnce({ exitCode: 1, stdout: '', stderr: 'FAILURE: Build failed', duration: 3000, command: './gradlew bundleRelease' })
  ...
  it('returns failure when no .aab found after build', async () => {
    const exec = vi.fn().mockResolvedValue({ exitCode: 0, stdout: 'BUILD SUCCESSFUL', stderr: '', duration: 5000, command: 'mock' })
