...
describe('buildKotlin', () => {
  it('runs gradlew assembleDebug and returns success', async () => {
    const exec = vi.fn().mockResolvedValue({ exitCode: 0, stdout: 'BUILD SUCCESSFUL', stderr: '', duration: 5000 })
    ...
    const gradleCall = exec.mock.calls.find((c: string[]) => c[0] === './gradlew' || c[1]?.[0] === 'assembleDebug')
  ...
  it('returns failure with parsed errors when build fails', async () => {
    const exec = vi.fn()
      ...
      .mockResolvedValueOnce({ exitCode: 1, stdout: 'app/src/main/java/com/dtc/app/HomeScreen.kt:42:5: error: Unresolved reference: LazyColumn', stderr: 'FAILURE', duration: 3000 })
    const runner = mockRunner({ exec, exists: vi.fn().mockResolvedValue(true) })
  ...
  it('creates build.gradle.kts when missing', async () => {
    ...
    const buildGradleWrite = writeFile.mock.calls.find((c: string[]) => c[0].includes('build.gradle.kts'))
