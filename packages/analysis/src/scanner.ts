...
const COMPONENT_SUFFIXES =
  /(?:Row|Cell|Button|Card|Badge|Label|Icon|Item|Header|Footer|Overlay|Shape|Style|Indicator|Tag|Chip|Segment|Toggle|Picker|Slider|Bar|Tab|Banner|Divider|Avatar|Thumbnail|Preview)$/

/** Content patterns that indicate a View struct is a full screen */
const SCREEN_CONTENT_MARKERS =
  /NavigationStack|NavigationView|NavigationSplitView|\.navigationTitle|\.navigationBarTitle|TabView|\.sheet\(|\.fullScreenCover\(|\.toolbar\s*\{/

const SWIFTUI_RULES: ClassificationRule[] = [
  { pattern: /struct\s+(\w+)\s*:\s*View\b/, type: 'screen', namePattern: /struct\s+(\w+)/ },
  {
    pattern: /@Observable\s+class\s+(\w+)|class\s+(\w+)\s*:\s*ObservableObject/,
    type: 'component',
    namePattern: /class\s+(\w+)/,
  },
  {
    pattern: /struct\s+(\w+)\s*:\s*(?:.*(?:Identifiable|Codable))/,
    type: 'model',
    namePattern: /struct\s+(\w+)/,
  },
  {
    pattern: /class\s+(\w+Service)\b|struct\s+(\w+Service)\b/,
    type: 'service',
    namePattern: /(?:class|struct)\s+(\w+)/,
  },
]
...
const KOTLIN_RULES: ClassificationRule[] = [
  ...
  {
    pattern: /class\s+(\w+(?:Repository|Service))\b/,
    type: 'service',
    namePattern: /class\s+(\w+)/,
  },
]
