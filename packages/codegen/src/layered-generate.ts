...
export function createLayeredGenerateFn(
  opts: LayeredGenerateOpts,
): (input: CodegenInput) => Promise<LayeredCodegenResult> {
    ...
    try {
      ...
      if (allFiles.length === 0) {
        return {
          ...
          tokensUsed,
          error:
            'Could not parse files from LLM response. Raw response saved to codegen-response.txt',
          presentationFiles: [],
          domainFiles: [],
          integrationFiles: [],
          presentationTokens: tokensUsed,
          domainTokens: 0,
          integrationTokens: 0,
        }
      }
      ...
      const integrationFiles = allFiles.filter(
        (f) =>
          f.path.includes('/ViewModels/') ||
          f.path.includes('/viewmodels/') ||
